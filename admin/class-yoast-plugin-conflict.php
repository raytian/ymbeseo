...
class Yoast_Plugin_Conflict {
	...
	protected function set_error( $plugin_section, $readable_plugin_section ) {
		...
		/* translators: %1$s: 'Facebook & Open Graph' plugin name(s) of possibly conflicting plugin(s), %2$s to YMBE SEO */
		$error_message .= '<p>' . sprintf( __( 'The %1$s plugin(s) might cause issues when used in conjunction with %2$s.', 'ymbeseo' ), $plugins_as_string, 'YMBE SEO' ) . '</p>';
		$error_message .= sprintf( $readable_plugin_section, 'YMBE SEO', $plugins_as_string ) . '<br/><br/>';
		$error_message .= '<p><strong>' . __( 'Recommended solution', 'ymbeseo' ) . '</strong><br/>';

		/* translators: %1$s: 'Facebook & Open Graph' plugin name(s) of possibly conflicting plugin(s). %2$s to YMBE SEO */
		$error_message .= sprintf( __( 'We recommend you deactivate %1$s and have another look at your %2$s configuration using the button above.', 'ymbeseo' ), $plugins_as_string, 'YMBE SEO' ) . '</p>';
		...
		$error_message .= '<p class="alignright"><small>' . sprintf( __( 'This warning is generated by %1$s.', 'ymbeseo' ), 'YMBE SEO' ) . '</small></p><div class="clear"></div>';
