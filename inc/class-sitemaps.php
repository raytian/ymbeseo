...
class YMBESEO_Sitemaps {
	...
	function build_post_type_map( $post_type ) {
		...
		if ( $n === 1 ) {
			...
			/**
			 * Filter: 'ymbeseo_sitemap_post_type_archive_link' - Allow changing the URL YMBE SEO uses in the XML sitemap for this post type archive.
			 *
			...
			if ( $archive_url ) {
				/**
				 * Filter: 'ymbeseo_xml_post_type_archive_priority' - Allow changing the priority of the URL YMBE SEO uses in the XML sitemap.
				 *
		...
		while ( $total > $offset ) {
			...
			if ( is_array( $posts ) && $posts !== array() ) {
				foreach ( $posts as $p ) {
					...
					/**
					 * Filter: 'ymbeseo_xml_sitemap_post_url' - Allow changing the URL YMBE SEO uses in the XML sitemap.
					 *
	...
	function output() {
		...
		echo $this->sitemap;
		echo "\n", '<!-- XML Sitemap generated by YMBE SEO -->';
	...
	private function calculate_priority( $post ) {
		 ...
		 * YMBE SEO uses in the XML sitemap.
		 *
		 * @api float $priority The priority for this URL, ranging from 0 to 1
		 *
		 * @param string $post_type The post type this archive is for.
